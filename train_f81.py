...
[훈련 방식]
  - WindowedSimF81Dataset: 각 위치를 중심으로 481bp 윈도우 슬라이딩
  - Center-site loss: 각 윈도우의 중앙 위치(index 240)에만 F81 loss 계산
    → 481bp 맥락을 보고 중앙 위치의 F81 θ를 예측하도록 학습
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--stride",     type=int, default=1,
                   help="슬라이딩 윈도우 보폭 (1=모든 위치, 클수록 빠름)")
...
def main() -> None:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            valid_mask = batch["valid_mask"].to(device)   # (B, W)
            center_idx = batch["center_idx"].to(device)   # (B,)
            B          = input_ids.shape[0]

            # center-site loss: 중앙 위치만 True
            valid_mask_center = torch.zeros_like(valid_mask)
            valid_mask_center[torch.arange(B), center_idx] = True

            optimizer.zero_grad()
            logits_dict = model(input_ids)   # {'A','C','G','T'}: (B, W_out)

            loss = loss_fn(
                logits_dict = logits_dict,
                msa_codes   = msa_codes,
                valid_mask  = valid_mask_center,
            )
            loss.backward()
            optimizer.step()
        ...
        if valid_loader is not None:
            ...
            with torch.no_grad():
                for batch in valid_loader:
                    ...
                    loss = loss_fn(logits_dict, msa_codes, valid_mask_center)
                    val_loss += loss.item()
                    val_n    += 1
