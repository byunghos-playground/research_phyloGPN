"""
train_f81_supervised_gc.py

[Exp3 / Exp4] 모델 F81 Supervised 훈련 — GC continuous variation 데이터용.

[baseline train_f81_supervised.py와의 차이]
  - WindowedSimF81Dataset 사용: 긴 genome npz → sliding window로 분해
  - π_true는 position마다 다름 (GC OU process 기반 continuous variation)

Usage:
  python train_f81_supervised_gc.py \\
    --data_dir    data/exp3_gc/processed \\
    --tree_path   data/trees/241-mammalian-2020v2.1.nh.txt \\
    --out_dir     checkpoints/exp3_gc/f81_supervised \\
    --stride      1
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="PhyloGPN 훈련 (F81 supervised, GC sliding window).")
    ...
    p.add_argument("--out_dir",      type=str, default="checkpoints/exp3_gc/f81_supervised")
...
def setup_logger(out_dir):
    logger = logging.getLogger("train_sup_gc")
...
def main():
    ...
    train_ds = WindowedSimF81Dataset(
        ...
        window_size=args.window_size, use_msa=True, stride=args.stride,
    )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            loss = loss_fn(
                logits_dict=logits_dict, msa_codes=msa_codes,
                pi_true=pi_true, valid_mask=valid_mask,
            )
            loss.backward()
            optimizer.step()
